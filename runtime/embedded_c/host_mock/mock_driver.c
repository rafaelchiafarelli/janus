...
static uint16_t g_busy_remaining = 0;
...
void mock_driver_set_busy_for(uint16_t count) {
    g_busy_remaining = count;
}

static void record(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t *pixels) {
    ...
    entry->w = w;
    entry->h = h;
    entry->sample_pixel = (pixels != NULL) ? pixels[0] : 0;
}
...
bool display_busy(void) {
    if (g_busy_remaining == 0) return false;
    g_busy_remaining--;
    return true;
}
